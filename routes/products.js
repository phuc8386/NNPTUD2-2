...
function validateListQuery(req, res, next) {
  try {
    const page = parsePositiveInt(req.query.page, 'page');
...
function validateCreateProduct(req, res, next) {
  ...
  if (isBlank(title) || isBlank(price) || isBlank(description) || category === undefined || category === null || images === undefined || images === null) {
    return res.status(400).send({
      message: 'title, price, description, category, images are required'
    });
  }
  ...
  if (!slug) {
    return res.status(400).send({
      message: 'title is invalid to generate slug'
    });
  }
...
//getall
router.get('/', validateListQuery, function (req, res, next) {
  const { title, slug, minPrice, maxPrice, page, limit } = req._validatedQuery;
  const titleQ = title.toLowerCase();
  const slugQ = slug.trim();

  let result = data.filter(function (e) {
    if (e.isDeleted) return false;
    if (titleQ && typeof e.title === 'string' && !e.title.toLowerCase().includes(titleQ)) return false;
    if (slugQ && e.slug !== slugQ) return false; // slug equality check
...
// get by slug (exact match)
router.get('/slug/:slug', function (req, res, next) {
  const slug = req.params.slug;
  const result = data.find(function (e) {
...
router.post('/', validateCreateProduct, function (req, res, next) {
